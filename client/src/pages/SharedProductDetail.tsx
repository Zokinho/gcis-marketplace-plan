      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
                      ...
                      {validImages.map(({ url, index }) => (
                        <button
                          ...
                          className={`h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg border-2 transition ${
                            index === selectedImage ? 'border-brand-teal' : 'border-transparent hover:border-gray-300 dark:hover:border-slate-600'
                          }`}
                        >
                          <ProductImage
                            src={url}
            ...
            {/* CoA PDF download — shown when PDF is available but no test results */}
            {product.coaPdfUrl && !product.testResults && (
              <div className="rounded-lg border border-subtle surface p-6">
                <h2 className="mb-4 text-sm font-bold uppercase tracking-wide text-muted">
                  Certificate of Analysis
                </h2>
                <a
                  href={token ? getSharedProductPdfUrl(token, product.id) : '#'}
                  target="_blank"
