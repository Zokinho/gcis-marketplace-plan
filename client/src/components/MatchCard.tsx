...
function getScoreColor(score: number): string {
  ...
  return 'bg-red-100 text-brand-coral';
}
...
export default function MatchCard({ match, onDismiss, dismissing }: Props) {
  ...
  return (
    <div className="rounded-lg border bg-white p-4 transition hover:shadow-sm">
      <div className="mb-3 flex items-start justify-between">
        <div className="min-w-0 flex-1">
          <Link
            ...
            className="text-base font-semibold text-brand-dark hover:text-brand-blue transition"
          >
            {product.name}
          </Link>
        </div>
      ...
      <div className="mb-3 flex flex-wrap gap-x-5 gap-y-1 text-sm text-gray-500">
        ...
        <div className="mb-3 flex flex-wrap gap-1.5">
          {insights.slice(0, 3).map((insight, i) => {
            const color = insight.type === 'positive' ? 'bg-brand-sage/20 text-brand-sage'
              ...
              : insight.type === 'warning' ? 'bg-red-50 text-brand-coral'
              : 'bg-brand-offwhite text-brand-dark';
            return (
      ...
      <div className="flex gap-2">
        ...
        {onDismiss && (
          <button
            ...
            disabled={dismissing}
            className="rounded-lg border px-3 py-1.5 text-xs font-medium text-brand-dark transition hover:bg-brand-offwhite disabled:opacity-50"
          >
            Not Interested
          </button>
        )}
